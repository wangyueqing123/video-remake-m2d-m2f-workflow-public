#!/usr/bin/env python3
"""Verify an isolated R6.12 revision and its preserved R6.11 history."""
...
RECEIPT_PATH = "artifacts/P6/R611_TO_R612_COMPLETION_PAIR_RECEIPT.json"
...
def validate(project: Path) -> list[str]:
    ...
    revision = state.get("r612_revision") if isinstance(state.get("r612_revision"), dict) else {}
    receipt_path = project / RECEIPT_PATH
    receipt = load_json(receipt_path)
    if state.get("skill_version") != "R6.12":
        issues.append("PROJECT_VERSION_NOT_R612")
    if receipt.get("schema_version") != "R6.12-R611-COMPLETION-PAIR-REVISION-1.0" or receipt.get("status") != "PASSED":
        issues.append("R612_REVISION_RECEIPT_INVALID")
    if (receipt.get("source_skill_version"), receipt.get("destination_skill_version")) != ("R6.11", "R6.12"):
        issues.append("R612_REVISION_TRANSITION_INVALID")
    if receipt.get("historical_media_storage_strategy") != "FULL_COPY_ISOLATED":
        issues.append("R612_REVISION_COPY_ISOLATION_NOT_DECLARED")
    if receipt.get("external_calls_during_revision") != 0 or receipt.get("provider_calls_authorized") is not False:
        issues.append("R612_REVISION_EXTERNAL_CALL_OR_AUTHORITY_INVALID")
    ...
    history_plan = resolve(project, revision.get("source_scene_plan_relative_path"), "HISTORY_PLAN", issues)
    ...
    if history_state is not None:
        ...
        if history.get("skill_version") != "R6.11":
            issues.append("HISTORY_STATE_VERSION_MISMATCH")
    if history_plan is not None:
        digest = sha256_file(history_plan)
        if digest != revision.get("source_scene_plan_sha256") or digest != receipt.get("source_scene_plan_sha256"):
            issues.append("HISTORY_PLAN_HASH_MISMATCH")

    if inventory_path is not None:
        if sha256_file(inventory_path) != revision.get("source_inventory_file_sha256"):
        ...
        if inventory.get("file_count") != receipt.get("source_inventory_file_count") or canonical_sha256(rows) != receipt.get("source_inventory_sha256"):
        ...
        for row in rows:
            ...
            elif relative == "artifacts/P4/SCENE_PLAN.json":
                actual = history_plan
            else:
                actual = resolve(project, relative, "HISTORY_SNAPSHOT", issues)
    ...
    bound = state.get("artifacts", {}).get("P6", {}).get("R611_TO_R612_COMPLETION_PAIR_RECEIPT", {})
    if bound.get("relative_path") != RECEIPT_PATH or bound.get("sha256") != sha256_file(receipt_path) or bound.get("validation_status") != "VALIDATED":
        issues.append("R612_REVISION_RECEIPT_NOT_BOUND")
