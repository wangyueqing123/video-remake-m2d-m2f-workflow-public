#!/usr/bin/env python3
"""Verify an R6.9 immutable R6.8 snapshot plus append-only live ledgers."""
...
RECEIPT_PATH = "artifacts/P6/R68_TO_R69_DIAGNOSTIC_STYLE_RECEIPT.json"
...
def validate(project: Path) -> list[str]:
    ...
    if state.get("skill_version") != "R6.9":
        issues.append("PROJECT_VERSION_NOT_R69")
    if receipt.get("schema_version") != "R6.9-R68-DIAGNOSTIC-STYLE-REVISION-1.0" or receipt.get("status") != "PASSED":
        issues.append("R69_REVISION_RECEIPT_INVALID")
    if (receipt.get("source_skill_version"), receipt.get("destination_skill_version")) != ("R6.8", "R6.9"):
        issues.append("R69_REVISION_TRANSITION_INVALID")
    if receipt.get("external_calls_during_revision") != 0 or receipt.get("provider_calls_authorized") is not False:
        issues.append("R69_REVISION_EXTERNAL_CALL_OR_AUTHORITY_INVALID")
    ...
    if history_state is not None:
        ...
        if digest != portable.get("source_state_sha256") or digest != receipt.get("source_manifest_sha256"):
            issues.append("HISTORY_STATE_HASH_MISMATCH")
        history = load_json(history_state)
        if history.get("skill_version") != "R6.8":
    ...
    for state_key, receipt_key in ledger_map.items():
        ...
        current_prefix = [canonical_sha256(row) for row in rows[: len(preserved)]]
        if len(rows) < len(preserved) or current_prefix != preserved:
            issues.append(f"{state_key.upper()}_PRESERVED_PREFIX_CHANGED")

    bound = state.get("artifacts", {}).get("P6", {}).get("R68_TO_R69_DIAGNOSTIC_STYLE_RECEIPT", {})
    if bound.get("relative_path") != RECEIPT_PATH or bound.get("sha256") != sha256_file(receipt_path) or bound.get("validation_status") != "VALIDATED":
        issues.append("R69_REVISION_RECEIPT_NOT_BOUND")
