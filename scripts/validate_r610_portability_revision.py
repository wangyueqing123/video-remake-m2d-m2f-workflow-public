#!/usr/bin/env python3
"""Verify an R6.10 immutable R6.9 snapshot plus append-only live ledgers."""
...
RECEIPT_PATH = "artifacts/P6/R69_TO_R610_RECEIPT_LOCK_RECEIPT.json"
...
def validate(project: Path) -> list[str]:
    ...
    revision = state.get("r610_revision") if isinstance(state.get("r610_revision"), dict) else {}
    receipt_path = project / RECEIPT_PATH
    receipt = load_json(receipt_path)
    if state.get("skill_version") != "R6.10":
        issues.append("PROJECT_VERSION_NOT_R610")
    if receipt.get("schema_version") != "R6.10-R69-RECEIPT-PATH-LOCK-REVISION-1.0" or receipt.get("status") != "PASSED":
        issues.append("R610_REVISION_RECEIPT_INVALID")
    if (receipt.get("source_skill_version"), receipt.get("destination_skill_version")) != ("R6.9", "R6.10"):
        issues.append("R610_REVISION_TRANSITION_INVALID")
    if receipt.get("external_calls_during_revision") != 0 or receipt.get("provider_calls_authorized") is not False:
        issues.append("R610_REVISION_EXTERNAL_CALL_OR_AUTHORITY_INVALID")
    ...
    if history_state is not None:
        ...
        if history.get("skill_version") != "R6.9":
    ...
    bound = state.get("artifacts", {}).get("P6", {}).get("R69_TO_R610_RECEIPT_LOCK_RECEIPT", {})
    if bound.get("relative_path") != RECEIPT_PATH or bound.get("sha256") != sha256_file(receipt_path) or bound.get("validation_status") != "VALIDATED":
        issues.append("R610_REVISION_RECEIPT_NOT_BOUND")
