#!/usr/bin/env python3
"""Validate an assembled master, emit its R6.13 certificate, and close the project."""
...
CERTIFICATE_RELATIVE_PATH = "artifacts/P8/FINAL_RELEASE_CERTIFICATE_R613.json"
...
def validate_release(project: Path, state: dict[str, Any], qc: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    issues: list[str] = []
    if state.get("current_phase") != "P8" or state.get("status") not in {"ACTIVE", "COMPLETE"}:
...
def finalize(project: Path, qc_relative_path: str, release_name: str, commit: bool) -> dict[str, Any]:
    ...
    certificate = {
        "schema_version": "R6.13-FINAL-RELEASE-CERTIFICATE-1.0",
        "status": "PASSED",
        "release_name": release_name,
        "project_id": state.get("project_id"),
        "source_skill_version": state.get("skill_version"),
        "certification_tool_version": "R6.13",
    ...
    state.setdefault("artifacts", {}).setdefault("P8", {})["FINAL_RELEASE_CERTIFICATE_R613"] = {
        "relative_path": CERTIFICATE_RELATIVE_PATH,
        "sha256": cert_hash,
        "bytes": cert_path.stat().st_size,
        "validator": "finalize_r613_release.py",
...
def main() -> int:
    ...
    parser.add_argument("--final-qc", default="artifacts/P8/FINAL_MASTER_QC.json")
    parser.add_argument("--release-name", default="Video Remake Workflow R6.13 Production Final")
